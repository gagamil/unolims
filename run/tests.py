...
class RunCreateTestCase(TestCase):
    ...
    def test_build_wells(self):
        ...
        # wells = create_well_plate_template(barcodes=[bc.barcode for bc in barcodes], replication=RUN_REPLICATION__DUPLICATE, targets=RUN_METHOD_TARGETS__SALIVECLEAR)
        ...
        # wells = create_well_plate_template(barcodes=[bc.barcode for bc in barcodes], replication=RUN_REPLICATION__TRIPLICATE, targets=RUN_METHOD_TARGETS__SALIVECLEAR)
    ...
    def test_create_run_tepmlate_ok(self):
        ...
        template_wells = create_well_plate_template(barcodes=[bc.barcode for bc in barcodes], replication=RUN_REPLICATION__DUPLICATE, targets=RUN_METHOD_TARGETS__SALIVECLEAR)
        self.assertEqual(len(barcodes)*len(RUN_METHOD_TARGETS__SALIVECLEAR)*2, len(template_wells))

        template_wells = create_well_plate_template(barcodes=[bc.barcode for bc in barcodes], replication=RUN_REPLICATION__TRIPLICATE, targets=RUN_METHOD_TARGETS__SALIVECLEAR)
